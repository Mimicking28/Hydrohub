...
class UpdateRefilled extends StatefulWidget {
  const UpdateRefilled({super.key});

  @override
  State<UpdateRefilled> createState() => _UpdateStockState();
}

class _UpdateStockState extends State<UpdateRefilled> {
  List<dynamic> stocks = [];
  bool isLoading = true;
  ...
  Future<void> fetchStocks() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        // ✅ Only show refilled stocks
        stocks = allStocks.where((s) => s["stock_type"] == "refilled").toList();
        isLoading = false;
      });
    } else {
      setState(() {
              ...
              TextButton(
                onPressed: () {
                  final updatedData = {
                    ...
                    "size": selectedSize,
                    "amount": amount,
                    "stock_type": "refilled", // ✅ always refilled
